...
typedef struct GC_Header {
    ...
    size_t size;
    bool marked;
    void* forwarding; // For moving GC (Nursery -> Old)
} GC_Header;
...
#define NURSERY_SIZE (32 * 1024 * 1024) // 32MB

void gc_scan_region(void* start, void* end);
void gc_mark(void* ptr);

void* gc_evacuate(void* ptr) {
    if (!ptr || (char*)ptr < nursery_start || (char*)ptr >= nursery_end) return ptr;
...
void gc_mark(void* ptr) {
    if (!ptr || ptr < gc_min_ptr || ptr > gc_max_ptr) return;
    // Skip nursery objects as they are handled by evacuation
    ...
    while (h) {
        ...
        if (ptr >= obj_ptr && ptr < (void*)((char*)obj_ptr + h->size)) {
            if (!h->marked) {
                h->marked = true;
                if (getenv("DEBUG_GC")) printf("DEBUG GC: Marking %p (size %zu)\n", obj_ptr, h->size);
    ...
    while (*node) {
        ...
        if (!curr->marked) {
            *node = curr->next;
            ...
            if (obj_ptr < new_min) new_min = obj_ptr;
            if ((char*)obj_ptr + curr->size > (char*)new_max) new_max = (char*)obj_ptr + curr->size;
            node = &(*node)->next;
            kept++;
        }
    }
...
void* gc_alloc(size_t size) {
    ...
    if (nursery_top + total_size <= nursery_end) {
        ...
        void* obj_ptr = (void*)(h + 1);
        if (obj_ptr < gc_min_ptr) gc_min_ptr = obj_ptr;
        if ((char*)obj_ptr + size > (char*)gc_max_ptr) gc_max_ptr = (char*)obj_ptr + size;
        return obj_ptr;
    }
...
char* TypeChecker_checkAssign(void* _self, void* expr) {
    ...
    if (({ 
        ...
        if (({ 
            strcmp(((Header*)header)->type, "GetExpr") == 0;
        })) {
...
void TypeChecker_checkIf(void* _self, void* stmt) {
    ...
    if (({ 
        ...
        TypeChecker_reportError(_self, BNK_RT_Strings_concat(NULL, "If condition must be bool, got ", condType), ((IfStmt*)ifStmt)->condition);
    }
...
void TypeChecker_checkWhile(void* _self, void* stmt) {
    ...
    if (({ 
        ...
        TypeChecker_reportError(_self, BNK_RT_Strings_concat(NULL, "While condition must be bool, got ", condType), ((WhileStmt*)w)->condition);
    }
...
void CTranspiler_emit(void* _self, char* text) {
    struct CTranspiler* self = (struct CTranspiler*)_self;
    (void)self;
    ((CTranspiler*)_self)->output = BNK_RT_Strings_concat(NULL, ((CTranspiler*)_self)->output, text);
    end:
    (void)&&end;
    return;
}
    ...
    while (1) {
        ...
        if (({ 
            ...
            if (({ 
                ...
                Parser_advance(_self);
                Parser_consume(_self, "KEYWORD", "Entity");
                Parser_consume(_self, "SYMBOL", ".");
                break;
            }
            if (({ 
...
int main(int argc, char** argv) {
    ...
    if (({ 
        ...
        char* c_code = ((CTranspiler*)transpiler)->output;
        printf("%s\n", "--- Generated C Code ---");
        printf("%s\n", c_code);
        printf("%s\n", "------------------------");
        ...
        if (({ 
            ...
            File_writeString(outFile, c_code);
            File_close(outFile);
            printf("%s\n", BNK_RT_Strings_concat(NULL, "Saved to: ", out_path));
        } else {
